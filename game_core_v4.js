...
const graph = {
  ...
  room201: { corridor: 6 },
  room202: { corridor: 7 },
  bath: { corridor: 12, toilet: 6 },
  toilet: { bath: 6, corridor: 8 },
...
const areaLabels = {
  home: '自宅', town: '田舎町', lobby: '帳場', kitchen: '厨房', corridor: '客室廊下', room201: '201号室', room202: '202号室', bath: '浴場前', toilet: 'トイレ', archive: '宿帳庫', north: '北廊下', detached: '離れ通路'
};
...
function loadAssetTexture(path){
  const tex = assetLoader.load(path);
  tex.colorSpace = THREE.SRGBColorSpace;
  tex.anisotropy = 4;
  return tex;
}
const spriteSheets = {
  villager: loadAssetTexture('assets/char_beige.png'),
  okami: loadAssetTexture('assets/char_blue.png'),
  maid: loadAssetTexture('assets/char_button.png'),
  guide: loadAssetTexture('assets/char_hoodie.png'),
  guest: loadAssetTexture('assets/char_button.png'),
  chef: loadAssetTexture('assets/char_blue.png'),
  hero: loadAssetTexture('assets/char_beige.png'),
  toiletGuy: loadAssetTexture('assets/char_crouch.png')
};
const photoRefs = {
  exterior: loadAssetTexture('assets/ryokan_exterior_night.jpg'),
  corridor: loadAssetTexture('assets/corridor_ref.jpg'),
  room201: loadAssetTexture('assets/room201_ref.jpg'),
  room202: loadAssetTexture('assets/room202_ref.jpg'),
  forbidden: loadAssetTexture('assets/forbidden_ref.jpg'),
  wanted: loadAssetTexture('assets/wanted_poster.jpg'),
  search: loadAssetTexture('assets/search_poster.jpg'),
  face: loadAssetTexture('assets/face_selfie.png')
};
function addPhotoBackdrop(tex, opts){
  ...
  plane.position.set(opts.x||0, opts.y||opts.height/2, opts.z||0);
  plane.rotation.y = opts.ry || 0;
  plane.rotation.x = opts.rx || 0;
  plane.renderOrder = 0;
  areaGroup.add(plane);
  return plane;
}
function addPoster(tex, x, y, z, ry, w, h){
  const plane = new THREE.Mesh(
    new THREE.PlaneGeometry(w || 1.2, h || 1.8),
    new THREE.MeshBasicMaterial({ map: tex, transparent: false })
  );
  plane.position.set(x, y||1.55, z);
  plane.rotation.y = ry || 0;
  plane.renderOrder = 2;
  areaGroup.add(plane);
  return plane;
}
function addPosterCluster(list){
  list.forEach(args=>addPoster(...args));
}
function makeSpriteCharacter(type){
  const g = new THREE.Group();
  const tex = spriteSheets[type] || spriteSheets.villager;
  const ratio = tex.image && tex.image.height ? tex.image.width / tex.image.height : 0.58;
  const height = type === 'toiletGuy' ? 1.45 : 1.85;
  const width = height * ratio;
  const mat = new THREE.MeshBasicMaterial({ map: tex, transparent: true, alphaTest: 0.08, side: THREE.DoubleSide });
  const plane = new THREE.Mesh(new THREE.PlaneGeometry(width, height), mat);
  plane.position.y = type === 'toiletGuy' ? 0.72 : 0.92;
  plane.castShadow = false;
  plane.receiveShadow = false;
  plane.userData.billboard = true;
  g.add(plane);
  const shadow = new THREE.Mesh(new THREE.CircleGeometry(type === 'toiletGuy' ? 0.45 : 0.42, 18), new THREE.MeshBasicMaterial({ color: 0x000000, transparent: true, opacity: 0.22 }));
  shadow.rotation.x = -Math.PI/2; shadow.position.y = -0.23; shadow.scale.set(1.3,0.7,1); g.add(shadow);
  return g;
}
...
function makeCharacter(type, costume){
  return makeSpriteCharacter(type);
}
...
function buildArea(areaId){
  ...
  else if (areaId === 'toilet') buildToilet();
...
function buildLobby(){
  ...
  wallSegment(7.95, 0, 0.14, 3.2, 14, materials.darkWood);
  receptionDesk();
  addPhotoBackdrop(photoRefs.exterior, { x: 0, y: 2.1, z: -5.82, width: 13.6, height: 5.8 });
  addPosterCluster([[photoRefs.search,-6.72,1.7,2.1,Math.PI/2,0.9,1.35],[photoRefs.wanted,6.72,1.7,1.2,-Math.PI/2,0.9,1.35]]);
...
function buildRoom201(){
  ...
  addLamp(0,0,0.8);
  addPhotoBackdrop(photoRefs.room201, { x: 0, y: 1.85, z: -4.24, width: 8.5, height: 4.8 });
  addPoster(photoRefs.search,4.24,1.7,-1.2,-Math.PI/2,0.8,1.2);
  addDoor('room201ToCorridor','客室廊下',0,4.18,1.1,'corridor',{x:0,z:-1.8,yaw:0},null,0xf0e7d1);
...
function buildRoom202(){
  ...
  addLamp(0,0,0.8);
  addPhotoBackdrop(photoRefs.room202, { x: 0, y: 1.85, z: -4.24, width: 8.5, height: 4.8 });
  addPoster(photoRefs.wanted,-4.24,1.7,-1.1,Math.PI/2,0.8,1.2);
...
function buildBath(){
  ...
  addPosterCluster([[photoRefs.search,-4.82,1.7,1.1,Math.PI/2,0.9,1.35],[photoRefs.wanted,4.82,1.7,-0.8,-Math.PI/2,0.9,1.35]]);
  addDoor('bathToCorridor','客室廊下',-4.35,0,1.2,'corridor',{x:6.2,z:1.8,yaw:Math.PI},'x',0xddeff3);
  addDoor('bathToToilet','トイレ',4.35,0,1.2,'toilet',{x:2.6,z:0,yaw:Math.PI/2},'x',0xded8cf);
...
function buildToilet(){
  scene.fog.color.set(0x0d1014); scene.fog.near = 10; scene.fog.far = 26;
  createFloor(9, 8, materials.tile, -0.1);
  createCeiling(9, 8, 0xd8d7d5);
  wallSegment(0,-3.95,9,3.2,0.14,materials.wallDark); wallSegment(0,3.95,9,3.2,0.14,materials.wallDark); wallSegment(-4.45,0,0.14,3.2,8,materials.wallDark); wallSegment(4.45,0,0.14,3.2,8,materials.wallDark);
  const sink = new THREE.Mesh(new THREE.BoxGeometry(1.8,0.9,0.62), new THREE.MeshStandardMaterial({ color: 0xb9bfc6, roughness: 0.9 }));
  sink.position.set(-2.6,0.45,-2.9); sink.castShadow = sink.receiveShadow = true; areaGroup.add(sink); addBoxCollider(-2.6,-2.9,1.8,0.62);
  const mirror = new THREE.Mesh(new THREE.PlaneGeometry(1.6,1.2), new THREE.MeshBasicMaterial({ color: 0x90a4b7, opacity: 0.9, transparent: true }));
  mirror.position.set(-2.6,1.8,-3.78); areaGroup.add(mirror);
  const divider = new THREE.Mesh(new THREE.BoxGeometry(0.08,2.4,2.6), materials.darkWood); divider.position.set(0.2,1.2,0.9); areaGroup.add(divider); addBoxCollider(0.2,0.9,0.08,2.6);
  const divider2 = divider.clone(); divider2.position.set(2.4,1.2,0.9); areaGroup.add(divider2); addBoxCollider(2.4,0.9,0.08,2.6);
  const stallBack = new THREE.Mesh(new THREE.BoxGeometry(3.8,2.4,0.08), materials.wallRose); stallBack.position.set(2.2,1.2,2.2); areaGroup.add(stallBack); addBoxCollider(2.2,2.2,3.8,0.08);
  const paperRoll = new THREE.Mesh(new THREE.CylinderGeometry(0.12,0.12,0.22,12), new THREE.MeshStandardMaterial({ color: 0xf2f0eb, roughness: 1 }));
  paperRoll.rotation.z = Math.PI/2; paperRoll.position.set(1.1,0.9,1.8); areaGroup.add(paperRoll);
  addLamp(-2.6,-1.2,0.7,0xe6eefb); addLamp(2.2,-0.2,0.6,0xffe2b7);
  addPosterCluster([
    [photoRefs.wanted,-4.32,1.75,-0.2,Math.PI/2,1.0,1.5],
    [photoRefs.search,-4.32,1.75,1.6,Math.PI/2,1.0,1.5],
    [photoRefs.wanted,4.32,1.75,-0.8,-Math.PI/2,1.0,1.5]
  ]);
  addNPC('toiletGuy','しゃがみ込む男','toiletGuy',0,2.0,1.5,Math.PI,npcInteract);
  addDoor('toiletToCorridor','客室廊下',-4.0,0,1.2,'corridor',{x:-7.2,z:2.2,yaw:Math.PI/2},'x',0xd9d1c6);
  addDoor('toiletToBath','浴場前',4.0,0,1.2,'bath',{x:3.2,z:0,yaw:-Math.PI/2},'x',0xded8cf);
}
...
function buildArchive(){
  ...
  archiveShelves();
  addPhotoBackdrop(photoRefs.forbidden, { x: 0, y: 1.85, z: 5.78, width: 13.2, height: 4.8, ry: Math.PI });
  addPosterCluster([[photoRefs.wanted,-6.72,1.7,2.8,Math.PI/2,0.95,1.4],[photoRefs.search,6.72,1.7,-2.8,-Math.PI/2,0.95,1.4]]);
...
function buildNorth(){
  ...
  addPosterCluster([[photoRefs.wanted,5.22,1.7,-1.6,-Math.PI/2,0.9,1.35],[photoRefs.search,-5.22,1.7,1.0,Math.PI/2,0.9,1.35]]);
...
function buildDetached(){
  ...
  addPosterCluster([[photoRefs.wanted,-7.72,1.7,-1.2,Math.PI/2,0.9,1.35],[photoRefs.search,7.72,1.7,1.2,-Math.PI/2,0.9,1.35]]);
...
function npcInteract(entity){
  ...
  } else if (entity.id === 'toiletGuy') {
    showDialogue([
      ['？？？', '……今は話しかけるな。壁のポスターの方を見ろ。', 'villager'],
      ['主人公', '見なかったことにしておこう。', 'hero']
    ], ()=>{});
  }
}
...
function makePortrait(face){
  ...
  const src = 'assets/face_selfie.png';
  card.style.background = `linear-gradient(180deg, rgba(255,255,255,.10), rgba(0,0,0,.18)), url(${src}) center/cover no-repeat`;
...
function handleHomeOverlayAction(){
  ...
  if (data.action === 'leave-home') { location.href = 'game_core_v4.html?spawn=town_day1&v=v4game'; return true; }
  if (data.action === 'sleep') { location.href = 'home_start_v4.html?mode=sleep1&v=v4home'; return true; }
  return false;
}
function setStep(id){
  state.step = id;
...
function goHomeNow(){
  ...
  stopChase();
  resetInput();
  location.href = 'home_start_v4.html?mode=sleep1&v=v4home';
}
...
function animate(now){
  ...
  dynamicGroup.traverse(obj => {
    if (obj.userData && obj.userData.billboard) {
      obj.lookAt(camera.position.x, obj.parent ? obj.position.y : obj.position.y, camera.position.z);
    }
  });
